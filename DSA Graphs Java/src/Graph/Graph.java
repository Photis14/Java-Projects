//Author: John Photis
//Purpose: This Java program implements detectConnectedComponent(), findPath() and hasCycle().
...
public class Graph {
	...
	public ArrayList<ArrayList<Integer>> detectConnectedComponent() {
		...
		for (int j = 0; j < nVerts; j++) {
			...
			if (vertexList[j].wasVisited == false) {
				...
				for (int k = 0; k < nVerts; k++){
					if (vertexList[k].wasVisited == true && visited[k] == false){
						temp.add(k);
						visited[k] = true;
					}
				}
		...
		for (int j = 0; j < nVerts; j++)
			vertexList[j].wasVisited = false;
		return result; // for compilation. You need to change it.
	}
	...
	public boolean findPath(int start, int end){
		...
		//displayVertex(0); // display it
		theQueue.add(start); // insert at tail
		int v2;
	...
	public static void main(String[] args) {
		 ...
		 */
		System.out.println(theGraph1.detectConnectedComponent());
		...
		if (theGraph1.findPath(a, b)) {
			System.out.println("These two vertices are connected");
		}else {
			System.out.println("These two vertices are not connected");
		}
		input.close();

		/*
		 * Your codes on finding cycle on Graph theGraph1 and theGraph3
		 */
		if (theGraph1.hasCycle()) {
			System.out.println("This graph contains cycles.");
		} else {
			System.out.println("This graph does not contain cycles.");
		}
